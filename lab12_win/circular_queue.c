...
void enqueue(queue_t *queue, int value)
{
    assert(queue!=NULL);
    //Creating node 
    node_t *new_rear = malloc(sizeof(node_t));
    new_rear->data = value;
    //If queue is empty
    if(queue->rear==NULL){
        new_rear->next=new_rear; //Point node to itself
    }else{
        new_rear->next=queue->rear->next; //Point new rear to head
        queue->rear->next = new_rear; //Point old rear to new rear
    }
    //Update queue
    queue->rear = new_rear;
    queue->size++;
...
_Bool front(const queue_t *queue, int *element)
{
    ...
    //Store value in head of queue in element
...
_Bool dequeue(queue_t *queue, int *element)
{
    ...
    if(queue->size==0){
        ...
        *element = queue->rear->next->data;
        node_t *delete_front = queue->rear->next;
